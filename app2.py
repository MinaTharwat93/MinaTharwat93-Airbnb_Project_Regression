...
def display_second_page():
    ...
    for i in range(1, 12):
        img_path = f"/mount/src/minatharwat93-airbnb_project_regression/static/images/uni{i}.png"  # Use relative path
    ...
    for column in df.select_dtypes(include="O").columns:
        ...
        sns.barplot(x=top_30.index, y=top_30.values, hue=top_30.index, palette='viridis', legend=False)
    ...
    for i in range(1, 9):
        img_path = f"/mount/src/minatharwat93-airbnb_project_regression/static/images/unic{i}.png"  # Use relative path
        if os.path.exists(img_path):
    ...
    sns.lineplot(data=df, x=df_year, y='price', hue='season', errorbar=None)  # Updated ci to errorbar
    ...
    st.code("""
    sns.barplot(x='room_type', y='minimum_nights', hue='season', data=df, errorbar=None)  # Updated ci to errorbar
    ...
    st.image("/mount/src/minatharwat93-airbnb_project_regression/static/images/qd6.png", caption="Room Types Distribution")

    # Price variation over time by room type
    st.subheader("How does the price of accommodations vary over time by room type?")
    st.code("""
    sns.lineplot(data=df, x=df_year, y='price', hue='room_type', errorbar=None)  # Updated ci to errorbar
